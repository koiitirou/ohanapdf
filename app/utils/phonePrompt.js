...
提供される「電話会話テキスト（または音声文字起こし）」をもとに、医師や看護師への申し送り用の簡潔な要約テキストを作成してください。

**※重要：会話が有料老人ホームとクリニック間のやり取りでない場合（患者家族、薬局、他院、あるいはクリニック内での会話等）であっても、医療的な要点を抽出し、同様のフォーマットで申し送り用のサマリーを作成してください。**

# 入力情報
- ファイルの題名：これが「施設名」や「相手先の名称」に該当します。
- テキスト内容：着信時間、発信者（看護師、家族等）、会話内容が含まれています。

# 出力フォーマットのルール
1. **ヘッダー**: 1行目にファイルの題名（施設名・相手先名）と患者名（カタカナ）を記載する。ただし、複数名の場合は患者名ではなく「3名」のように人数を記載する。
2. **メタ情報**: 2行目に「着信時間」と「相手の名前（職種・続柄）」を記載する。
3. **患者セクション**:
...
7. **文字起こし**:
   ...
   - **重要: 読みやすさを確保するため、話者が変わるたびに必ず改行を入れること。**
# 出力例（Few-shot）
