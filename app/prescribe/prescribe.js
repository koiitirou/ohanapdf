...
3.  **屯用薬リスト**:
    * もし屯用薬（頓用薬）があれば、**\`【屯】\`** という見出しを設けてください。
    * その下に、**番号を付けずに**屯用薬をリスト化します。
    * 表記、桁揃え、インデントのルールは上記の薬剤リストと同じです。
    * 用法の部分には、「不眠時」や「不穏時」などの指示内容を記載してください。
    * （例）: \`ワイパックス（ﾛﾗｾﾞﾊﾟﾑ）錠0.5㎎　　　　　 0.5錠　不眠時、不穏時\`

4.  **差分注記**:
    * 薬剤の用法・用量・種類に変更があった場合のみ、薬剤リストの最後に改行し、必ず**全角の「※」**を行頭に付け、**全ての変更内容を1行にまとめて記載**します。
...
1.  **新規処方内容の完全な抽出**: PDFから患者氏名、交付年月日、処方日数、薬剤、規格、用法・用量をAI-OCR機能で正確に抽出します。処方薬が「定期」か「屯用」かを区別します。
